...
void Macaron::initializeOverlay(int stage)
{
    // see BaseApp.cc
...
void Macaron::handleTimerEvent(cMessage *msg)
{
    if (msg == rpcTimer) {
...
bool Macaron::isSiblingFor(const NodeHandle& node,
...
NodeVector *Macaron::findNode(const OverlayKey& key,
...
void Macaron::finishOverlay()
{
    // remove this node from the overlay
    setOverlayReady(false);

    // save the statistics (see BaseApp)
    globalStatistics->addStdDev("Macaron: Dropped packets", numDropped);
}

// Return the max amount of siblings that can be queried about
int Macaron::getMaxNumSiblings()
{
    return 1;
}

// Return the max amount of redundant that can be queried about
int Macaron::getMaxNumRedundantNodes()
{
    return 1;
}


void Macaron::getNeighbors(const OverlayKey &neighborKey)
{
...
bool Macaron::handleRpcCall(BaseCallMessage *msg)
{
...
void Macaron::handleRpcTimeout(BaseCallMessage* msg,
...
void Macaron::handleRpcResponse(BaseResponseMessage* msg,
...
void Macaron::callbackNeighbors(const NodeHandle& neighborKey,
...
void Macaron::callbackTimeout(const OverlayKey &neighborKey)
{
